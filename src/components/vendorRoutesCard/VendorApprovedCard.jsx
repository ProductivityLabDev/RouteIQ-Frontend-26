...
export function VendorApprovedCard({ trip, trips, onEditClick, selectedTab, handleMapScreenClick, handleEditRoute, handleRouteMap }) {
            ...
            <div className='border-4 border-[#28A745] rounded-md p-3 md:p-4 mb-4'>
                <div className='flex justify-between items-center md:flex-row flex-col'>
                    <div className='flex space-x-4 items-center'>
                        ...
                        <p className='text-black text-md md:text-[22px] font-semibold'>{trip?.busNumber || trips[0]?.busNumber}</p>
                        <p className='text-xs md:text-[12px] text-white font-medium bg-[#28A745] px-2 py-1 rounded-[4px]'>{trip?.status || trips[0]?.status}</p>
                    </div>
                    <div>
                        <p className='font-semibold text-xs md:text-[14px] text-[#141516]/80 md:pt-0 pt-3'> {trip?.time || trips[0]?.time}</p>
                    </div>
                </div>
                        ...
                        <div className='flex flex-col space-y-3 md:space-y-6'>
                            <div className='flex space-x-2'>
                                ...
                                <div className='text-black'>
                                    <h6 className='text-xs md:text-[14px] font-semibold'>{trip?.pickup?.location || trips[0]?.location}</h6>
                                    <p className='font-normal text-[12px]'>{trip?.pickup?.address || trips[0]?.pickup?.address}</p>
                                </div>
                            </div>
                            <div className='flex space-x-2'>
                                <img src={locationicon} className='w-[25px] h-[25px] mt-2' alt="not found" />
                                <div className='text-black'>
                                    <h6 className='text-[14px] font-semibold'>{trip?.dropoff?.location || trips[0]?.dropoff?.location}</h6>
                                    <p className='font-normal text-[12px]'>{trip?.dropoff?.address || trips[0]?.dropoff?.address}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='flex flex-row gap-3 mt-2'>
                        <div className='flex flex-col relative'>
                            <div className='bg-[#fff] flex flex-col justify-end p-1.5 h-[75px] w-[120px] rounded-md text-center shadow-xl text-white'>
                                ...
                                <p className='text-[12.5px] font-[700] text-[#000]'>{trip?.driver?.name || trips[0]?.driver?.name}</p>
                                <img src={trip?.driver?.image || trips[0]?.driver?.image} className='rounded-full w-14 h-14 object-cover absolute -top-8 left-8' alt="not found" />
                            </div>
                            <p className='text-[#565656] text-[14px] font-medium pt-2'>No. of Persons: <span className='text-black font-semibold'>{trip?.noOfPersons || trips[0]?.noOfPersons}</span></p>
                        </div>
                        <div className='flex flex-col gap-3'>
                            {selectedTab === "Trip Planner" ?
                                ...
                                :
                                <Button className='text-[#fff] bg-[#C01824] border-[#C01824] text-[12px] w-[70px] rounded-[4px] py-0.5 flex items-center justify-center capitalize' variant='outlined' size='sm' onClick={handleRouteMap}><img src={distance} className='w-4 h-4 mr-0.5' alt="not found" />Map</Button>
                            }
                            {selectedTab === "Trip Planner" ?

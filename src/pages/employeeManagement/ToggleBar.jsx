...
const ToggleBar = () => {
    ...
    const handleClose = () => {
        setEdit(false);
        const t = typeof isOpen === 'number' ? terminals[isOpen] : null;
        if (t?.terminalId) {
            const d = new Date();
            dispatch(fetchTerminalDetail({ terminalId: t.terminalId, month: d.getMonth() + 1, year: d.getFullYear() }));
        }
        setEditDriver(null);
    };
                        ...
                        <div key={terminal.terminalId} className="w-full bg-white border-b border-gray-200 shadow-sm">
                            ...
                            {isOpen === index && (
                                <div className="w-full bg-white border-t border-gray-200 shadow-sm">
                                    <DriverTable terminalId={terminal.terminalId} handleEdit={handleEdit} />
                                </div>
                            )}

                            {balanceModal === index && (
                                <BalanceModal setBalanceModal={() => setBalanceModal(null)} />
                            )}
                        </div>
                    ))}
                </div>

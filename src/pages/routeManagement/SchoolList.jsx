...
export default function SchoolList({ institutes = [], handleMapScreenClick, handleEditRoute }) {
    ...
    const formatTime = (val) => {
        if (!val) return "-";
        const iso =
            val?.estimatedArrivalTime ||
            val?.EstimatedArrivalTime ||
            val?.stopEstimatedArrivalTime ||
            val?.StopEstimatedArrivalTime ||
            val;

        // If string with "T", take HH:MM directly
    ...
    useEffect(() => {
        ...
        const already = Array.isArray(routesByInstitute?.[openSchoolId]) && routesByInstitute[openSchoolId].length > 0;
        const isLoading = loading?.instituteRoutes?.[openSchoolId];
    ...
    const handleSchoolMap = (inst) => {
        const list = routesByInstitute?.[inst.instituteId];
        if (Array.isArray(list) && list.length > 0) {
            // Use the first route of this school to open map
            handleMapScreenClick(list[0]);
        } else {
    ...
    return (
        <div className="w-full p-4">
            {displayInstitutes.length === 0 ? (
                <div className="border border-gray-200 rounded-lg p-6 text-center text-gray-600 bg-white">
                    No schools assigned to this terminal.
                </div>
            ) : null}
            {displayInstitutes.map((inst, idx) => {
                const isOpen = openSchoolId === inst.instituteId;
                const logo = [SkylineSchoolLogo, LakeviewSchoolLogo, SpringdaleSchoolLogo, RosewoodSchoolLogo][idx % 4];
                const routes = routesByInstitute?.[inst.instituteId] || [];
                const routesLoading = loading?.instituteRoutes?.[inst.instituteId];

                return (
            <div key={inst.instituteId || idx} className="border border-gray-200 rounded-lg shadow-sm mb-3">
                {/* School Header */}
                <div className="p-4 flex items-center justify-between border-b border-gray-200">
                    <div className="flex items-center">
                        ...
                        <div className="font-bold text-lg">{inst.instituteName || "School"}</div>

                        <button
                            className="text-gray-600 hover:text-gray-800 ml-3"
                            onClick={() => setOpenSchoolId(isOpen ? null : inst.instituteId)}
                        >
                    ...
                    <div className="flex items-center">
                        <div className="mr-4">
                            <span className="font-bold">Total Students:</span> {inst.totalStudents ?? 0}
                        </div>

                        <div className="mr-4">
                            <span className="font-bold">Total Buses:</span> {inst.totalBuses ?? 0}
                        </div>

                        <button className="bg-[#C01824] text-white px-3 py-1 rounded mr-3 flex items-center" onClick={() => handleSchoolMap(inst)}>
                        ...
                        <button onClick={() => setOpenSchoolId(isOpen ? null : inst.instituteId)}>
                ...
                {isOpen && (
                    <div className="p-4">
                        {routesLoading && (
                            <div className="py-4 text-gray-600">Loading routes...</div>
                        )}
                        {!routesLoading && Array.isArray(routes) && routes.length === 0 && (
                            <div className="py-4 text-gray-600">No routes found for this institute.</div>
                        )}
                        {!routesLoading && Array.isArray(routes) && routes.length > 0 && routes.map((route, ridx) => (
                        <div key={route.routeId || ridx} className="mb-4">
                            <div className="border border-gray-200 rounded-md p-4 flex items-center justify-between">
                                <div className="flex items-center">
                                    ...
                                    <div className="font-bold flex items-center gap-2">
                                        <span>{route.routeNumber || `Route ${ridx + 1}`}</span>
                                        <span>{route.routeName ? `- ${route.routeName}` : ""}</span>
                                        <span role="img" aria-label="route-icon">
                                            {route.routeAnimalIcon || "🦊"}
                                        </span>
                                    </div>
                                    <button className="text-gray-600 hover:text-gray-800 ml-3" onClick={() => toggleRoute(`route-${route.routeId || ridx}`)}>
                                         <svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                           <path d="M3.27702 20.6328C3.01105 20.6328 2.78653 20.5278 2.60348 20.318C2.42056 20.1079 2.3291 19.8503 2.3291 19.5451V17.1006C2.3291 16.8114 2.3766 16.535 2.4716 16.2714C2.56646 16.0079 2.70195 15.7753 2.87806 15.5733L13.3191 3.61121C13.4924 3.42443 13.6848 3.28132 13.8962 3.18188C14.1074 3.08244 14.3292 3.03271 14.5614 3.03271C14.7903 3.03271 15.0139 3.08244 15.2322 3.18188C15.4504 3.28132 15.6426 3.43049 15.8087 3.62938L17.1687 5.19657C17.342 5.38334 17.4694 5.60295 17.5508 5.85538C17.6322 6.10765 17.6729 6.36096 17.6729 6.61531C17.6729 6.88177 17.6322 7.13715 17.5508 7.38145C17.4694 7.62592 17.342 7.8476 17.1687 8.04648L6.74848 20.0029C6.5725 20.205 6.36966 20.3604 6.13994 20.4693C5.91035 20.5783 5.66952 20.6328 5.41744 20.6328H3.27702ZM14.5556 7.94823L15.7222 6.61531L14.5506 5.26517L13.3789 6.60384L14.5556 7.94823Z" fill="#1C1B1F"/>
                                           </svg>
                                    </button>
                            ...
                            {expandedRoutes[`route-${route.routeId || ridx}`] && (
                                <div className="p-4">
                                    {/* Bus Info Card */}
                                    <div className="border-4 border-[#28A745] rounded-lg p-4 mb-6">
                                        <div className="flex justify-between">
                                            <div className="flex items-center">
                                                ...
                                                <div>
                                                   <span className="font-bold text-[20px] text-[#141516]">{route.vehicleNumberPlate || 'Vehicle'}</span> 
                                                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                   <span className="text-[20px] text-[#141516] font-bold">{route.routeNumber || ''}</span>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <span className="font-semibold text-[#141516] text-[14px]">{route.routeName || ''}</span>
                                            </div>
                                        </div>
                                        <div className='flex items-center justify-between'>
                                            ...
                                            <div className="flex items-center p-4 h-[120px] w-[250px] rounded-lg shadow-md">
                                                ...
                                                <p className="text-[18px] font-bold text-[#141516]">{route.driverName || 'Driver'}</p>
                                                <p className="text-[13px] text-[#7C7D7D] font-semibold">Driver</p>
                                                </div>
                                            </div>
                                            </div>
                                            <div className="flex flex-col items-center gap-2">
                                                <button className="bg-[#C01824] text-white px-6 py-1 rounded flex items-center" onClick={() => handleMapScreenClick(route)}>
                                    ...
                                    <div className="overflow-x-auto border border-gray-200 rounded-md">
                                        <table className="w-full border-collapse">
                                            ...
                                            <tbody>
                                                {loading?.routeStudents?.[route.routeId] ? (
                                                    <tr>
                                                        <td className="p-3 text-sm text-gray-600" colSpan={5}>Loading students...</td>
                                                    </tr>
                                                ) : errors?.routeStudents?.[route.routeId] ? (
                                                    <tr>
                                                        <td className="p-3 text-sm text-red-500" colSpan={5}>{errors.routeStudents[route.routeId]}</td>
                                                    </tr>
                                                ) : Array.isArray(studentsByRoute?.[route.routeId]) && studentsByRoute[route.routeId].length > 0 ? (
                                                    studentsByRoute[route.routeId].map((stu, idx) => (
                                                        <tr key={stu.studentId || idx} className="border-t border-gray-200">
                                                            <td className="p-3 font-semibold text-[#141516] text-[14]">
                                                                {route.routeId || route.RouteId || route.routeNumber || `Route ${ridx + 1}`}
                                                            </td>
                                                            <td className="p-3 text-xl">
                                                                {route.routeAnimalIcon || "🦊"}
                                                            </td>
                                                            <td className="p-3 font-semibold text-[#141516] text-[14]">
                                                                {stu.studentName || `Student ${idx + 1}`}
                                                            </td>
                                                            <td className="p-3 font-semibold text-[#141516] text-[14]">
                                                                {(() => {
                                                                    const rawTime =
                                                                        stu.estimatedArrivalTime ||
                                                                        stu.EstimatedArrivalTime ||
                                                                        stu.stopEstimatedArrivalTime ||
                                                                        stu.StopEstimatedArrivalTime ||
                                                                        stu.time ||
                                                                        stu.pickupTime;
                                                                    const formatted = formatTime(rawTime);
                                                                    return formatted === "-" && rawTime
                                                                        ? String(rawTime)
                                                                        : formatted;
                                                                })()}
                                                            </td>
                                                            <td className="p-3 whitespace-pre-line font-semibold text-[#141516] text-[14]">
                                                                {stu.studentAddress || stu.stopAddress || [stu.pickupCity, stu.pickupState].filter(Boolean).join(", ") || "-"}
                                                            </td>
                                                        </tr>
                                                    ))
                                                ) : (
                                                    <tr>
                                                        <td className="p-3 text-sm text-gray-600" colSpan={5}>No students found.</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>

...
export function ForgotPassword() {
  ...
  const requestPasswordReset = async (targetEmail) => {
    ...
    return axios.post(url, { email: targetEmail });
  };
      ...
      <section className="h-screen flex items-center justify-center bg-[url('./assets/auth-bg.png')] bg-cover bg-center bg-no-repeat px-2 md:px-4">
        <div className="w-full max-w-[600px] mx-auto py-7 md:py-16 md:px-7 px-4 rounded-xl bg-white">
          <div className="mx-auto w-full max-w-[500px]">
            {step === 1 && (
              <>
                <img
                  ...
                  alt="not found"
                />
                <Typography className="font-medium text-[#C01824] text-[24px] md:text-[45px] mb-1 mt-7">
                  Forgot Password
                </Typography>
            ...
            {step === 2 && (
              <>
                <Typography className="font-semibold my-3 text-[28px] md:text-[45px] text-[#C01824]">
                  Verification
                </Typography>
            ...
            {step === 3 && (
              <>
                <img
                  src={logo}
                  className="w-full max-w-[150px] md:max-w-[220px]"
                  alt="not found"
                />
                <Typography className="font-medium text-[24px] md:text-[45px] py-3 md:py-9 text-[#C01824]">
                  New Password
                </Typography>
                <Typography className="text-sm md:text-[16px] pb-3 text-[#2C2F32] font-medium">
                  Set the new password for your account so you can log in and
                  access all features.
                </Typography>
              </>
            )}
            {step === 4 && (
              <>
                <img
                  src={redtick}
                  alt="Tick mark"
                  className="w-28 h-28 mx-auto my-6"
                />
                <Typography className="font-medium my-3 text-[#C01824] text-[24px] md:text-[34px] text-center">
                  Successfully
                </Typography>
                <Typography
                  variant="paragraph"
                  className="text-[14px] md:text-[16px] font-normal text-center"
                >
          ...
          <form
            className="mb-2 mx-auto w-full max-w-[500px]"
            onSubmit={
              step === 1
                ? handleEmailSubmit
                : step === 2
          ...
          >
            {step === 1 && (
              <div className="mb-1 flex flex-col gap-5">
                ...
                <Button
                  type="submit"
                  className="mt-3 bg-[#C01824] text-[14px] md:text-[16px] font-semibold rounded-[5px] py-4 opacity-100"
                  fullWidth
                >
                  Continue
                </Button>
              ...
              <div className="mb-1 flex flex-col gap-3">
                <div className="grid grid-cols-4 place-content-center gap-1 place-items-center mt-8 mb-4">
                  {otp.map((value, index) => (
                    <div
                      key={index}
                      className="w-full max-w-[70px] md:max-w-[84px] md:h-[76px]"
                    >
                      <input
                ...
                <Button
                  type="submit"
                  className="mt-4 bg-[#C01824] font-normal text-[16px] rounded-[5px] py-4 opacity-100"
                  fullWidth
                ...
                <Typography className="text-center text-[14px] md:text-[16px] text-[#808080] mt-3">
                  ...
                  <button
                    ...
                    className="ml-1 font-medium text-[#C01824]"
                  >
                    Resend
                  </button>
                </Typography>
            ...
            {step === 3 && (
              <div className="my-5 flex flex-col gap-5">
                <Typography
                  variant="small"
                  className="-mb-3 font-medium text-[#2C2F32]"
                >
                  Enter new password
                </Typography>
                <div className="relative">
                  ...
                  >
                    {showNewPassword ? (
                      <EyeIcon className="h-[20px] w-[20px] text-[#0F1012]" />
                    ) : (
                      <EyeSlashIcon className="h-[20px] w-[20px] text-[#0F1012]" />
                    )}
                  </div>
                </div>
                <Typography
                ...
                <Button
                  type="submit"
                  className="mt-4 bg-[#C01824] font-normal text-[14px] md:text-[16px] rounded-[5px] py-4 opacity-100"
                  fullWidth
